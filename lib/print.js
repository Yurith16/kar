...
const loadDependencies = async () => {
  if (global.opts?.img) {
    try {
      ...
      console.error('Error cargando terminal-image:', e)
    }
  }
  try {
    const urlRegexSafe = require('url-regex-safe')
    urlRegex = urlRegexSafe({ strict: false })
  } catch (e) {
    console.error('Error cargando url-regex-safe:', e)
  }
}
loadDependencies()
...
watchFile(file, () => { 
  ...
  console.log(chalk.green.bold('│') + chalk.white(' 🌱 Update ') + chalk.greenBright.bold("'lib/print.js'") + chalk.white(' - KarBot 🍀'))
  console.log(chalk.green.bold('╰───────────────────···'))
})

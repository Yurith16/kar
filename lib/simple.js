            ...
            ),
            {
              ...
              isBaileys: {
                get() {
                  ...
                  const isMysticPattern = /^MYSTIC[A-F0-9]+$/.test(messageId);
                  return (
                    isMysticPattern || isSukiPattern || hasKnownPrefix || false
                  );
                },
                enumerable: true,
              },
                ...
                get() {
                  try {
                    ...
                    if (parsedJid && parsedJid.includes("@lid")) {
                      const groupChatId = this.chat?.endsWith("@g.us")
                        ? this.chat
                        : null;
                      if (groupChatId) {
                  ...
                  const processJid = (user) => {
                    try {
                      ...
                      if (
                        ...
                        const resolved =
                          String.prototype.resolveLidToRealJid.call(
                            user,
                            groupChatId,
                            self.conn
                          );
                        return resolved.then((res) =>
                          typeof res === "string" ? res : user
                        );
                      }

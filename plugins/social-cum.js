...
// Variable en memoria para llevar el orden sucesivo
let lastIndex = -1;
...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (!who) return m.reply(`> ¿Sobre quién piensas terminar hoy, cielo? Etiqueta a alguien para que el drama comience.`);
    ...
    // --- LISTA DE RECURSOS (Sucesión ordenada) ---
    const videos = [
    ...
    // Incrementamos el índice para que sea sucesivo
    lastIndex = (lastIndex + 1) % videos.length;
    const media = videos[lastIndex];

    try {
        await m.react('💦');
        
        // Enviamos el video de forma sucesiva con gifPlayback
        ...
        m.reply(`> 🥀 El momento fue tan fuerte que mi sistema perdió el aliento, amor. Inténtalo de nuevo.`);
    }
}

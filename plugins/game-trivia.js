...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    if (Date.now() - time < 15000) {
        ...
        return m.reply(`> ⏳ *DESPACIO:* Tu mente está ardiendo, espera *${wait}s* para la siguiente pregunta, cielo.`)
    }

    if (salasTrivia.has(id)) return m.reply(`> 🎀 *AVISO:* Ya tienes una trivia activa. ¡Responde con el número antes de que se enfríe nuestro momento!`)
    ...
    if (!category || !validCategories.includes(category)) {
        ...
        help += `> Elige una categoría para demostrar tu intelecto, mi vida:\n\n`
        validCategories.forEach(cat => help += `• *${cat.toUpperCase()}*\n`)
        help += `\n💡 *Uso:* *${usedPrefix + command} cultura*`
        return m.reply(help)
    }
...
handler.before = async (m, { conn }) => {
    ...
    if (input === game.correct) {
        ...
        win += `_Me encantan las mentes tan capaces como la tuya._ 💋`
        ...
        return m.reply(`🚫 *¡𝗜𝗡𝗖𝗢𝗥𝗥𝗘𝗖𝗧𝗢!*\n\n> La respuesta era: *${game.ans}*\n> Tu racha 🔥 se ha roto. Me has decepcionado un poquito, pero el fracaso también tiene su encanto... 💋`)
    }
    return true
}
